...
//    Verifica se há conectividade e, caso positivo, verifica se há nova versão disponível para downlaod

//	Determina a pasta local do GCP, que está uma acima do arquivo executável
	String exeFile = Application->ExeName;
	String exePath = ExtractFilePath(exeFile);
	exePath = exePath.SubString(1,exePath.Length()-2);
	String GCPPath = ExtractFilePath(exePath);
...
//    Carrega e analisa o arquivo da versao lastversion.txt
	try {
		...
		if (versao!=ultima_versao) {
			...
			if (Application->MessageBoxA(L"Existe uma versão mais nova do GCP disponível para download.\n Deseja atualizar agora?",L"Atenção",MB_YESNO)==ID_YES) {
